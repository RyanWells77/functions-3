...
// CODE HERE
numbers.forEach(element => console.log(element / 2)) // Why would I do this?
...
// CODE HERE
const numbersDivided = numbers.map(element => element / 2 )
...
// CODE HERE
const citysEndingWithI = cities.filter(name => name.endsWith `i`)
console.log(citysEndingWithI)
...
fruits.forEach((color, index) =>{ 
  if(color.color === "red"){
    console.log(`The fruit with index ${fruits.findIndex(fruit => fruit.name === "apple")} is and apple`)
    } else if(color.color === "orange"){
      console.log(`The fruit with index ${fruits.findIndex(fruit => fruit.name === "orange")} is and orange.`)       
    } else {
...
// CODE HERE
const riceFoods = foods.filter(element => element.tags.includes("rice")).reduce((runningTotal, price) => runningTotal +price.price, 0)
