...
// CODE HERE
const totalPopulation = populations.reduce((acc, curEle) => acc + curEle)
...
// CODE HERE
 const myStrongest  = monstersInYourPocket.filter(arrCp => arrCp.CP > 200)
...
const totalAfterTax = orders.map(price => (price.price *(1 + price.tax)))//.reduce(newPrice => newPrice + newPrice)
...
// CODE HERE
const sumOfBobsTotals = purchases.filter(owner => owner.owner === "Bob").reduce((acc, cost) => acc + cost.price,0) // Why do I have to set the accumulator to 0? shouldn't it stat with the first .price if left undefined?

console.log(sumOfBobsTotals)